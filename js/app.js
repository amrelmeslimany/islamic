...
light_btn.addEventListener("click", (e) => {
    ...
    if (document.querySelectorAll("[href='css/dark-style.css']")) {
        document
            .querySelectorAll("[href='css/dark-style.css']")
            .forEach((el) => {
                el.remove();
            });
    }
        ...
        all_sounds.forEach((sound) => {
            ...
            sound.addEventListener("click", function (c) {
                ...
                let radioLink = this.dataset.radio,
                    name_radio = this.querySelector(
                        "a.title"
                    ).textContent.trim();
                radio_audio.src = radioLink;
                play_btn.click();
                document.querySelector(
                    ".information-audio h4"
                ).innerHTML = name_radio;
            });
        });
    }
...
if (
    ...
    const books = document.querySelectorAll(
        ...
        book_dsc_info = document.querySelectorAll(
            ".books-libs .book-discrption"
        );
...
if (
                ...
                if (confirm(`متاكد (النجوم = ${this.dataset.rating}) ؟`)) {
                    ...
                    if (get_rating_local == null) {
                        localStorage.setItem(
                            "rating",
                            JSON.stringify(rating_array)
                        );
                    } else {
                        let strg_data = JSON.parse(
    ...
    all_headers.forEach((head_element) => {
        ...
        if (header_array.length > 3) {
            ...
            head_element.innerText =
                head_element.innerText.slice(0, 14) + "...";
        }
    });
    ...
    search_bx.querySelector(".search").addEventListener("click", function (e) {
        ...
        ancient_articles.forEach((anc_artcle) => {
            let anc_titles = anc_artcle.querySelector(".article-title a")
                .innerText;
...
if (location.href.search("azkar.html") > -1) {
    ...
    const azkar_btn = document.querySelectorAll(".footer-two .link-ch-ad"),
        ...
        minus_font = document.querySelector(
            ".bottom-nav .n-links-box .minus-down"
        ),
...
if (
                ...
                ARTICLESS.forEach((article) => {
                    if (article.title === title_link) {
                        ...
                        articles_text_outer.querySelector(
                            "h3"
                        ).textContent = title_link;
                        articles_text_inner.querySelector(
                            ".the-article-self"
                        ).innerHTML = article.body;
                        articles_text_outer.querySelector("p").innerHTML =
                            article.body;
                        ...
                        search_bx
                            ...
                            .addEventListener("click", (c) => {
                                c.preventDefault();
                                // ----------
                                const input_search_val = search_bx.querySelector(
                                    "input"
                                ).value;
                                let regex_search = new RegExp(
                                    `(${input_search_val})`,
                                    "g"
                                );
                                if (input_search_val != "") {
                                    if (
                                        ...
                                        search_founded_ele.textContent =
                                            findElements.length;
                                        search_founded_ele.parentElement.classList.add(
                                            "active"
                                        );
    ...
    articles_text_inner
        ...
        .addEventListener("click", function (c) {
            ...
            makeCopy(
                this,
                articles_text_inner.querySelector(".the-article-self")
            );
        });
...
if (location.href.search("allah-names.html") > -1) {
    const plus_btn = document.querySelector(".bottom-nav .link-nav.plus-up"),
        ...
        changes = document.querySelectorAll(".font-change-size"),
        search_bx = document.querySelector(".header .search-bx"),
        list_names_bx = document.querySelectorAll(
            ".allah-names-section .box-name"
        ),
        catch_all_definations = document.querySelector(
            ".catch-all-definations"
        ),
    ...
    search_bx
        ...
        .addEventListener("click", function (c) {
            ...
            list_names_bx.forEach((name_bx, i) => {
                let text_name = name_bx.querySelector(".allah-name")
                        .textContent,
    ...
    list_names_bx.forEach((bx) => {
        ...
        catch_all_definations.querySelectorAll(".box-def").forEach((df) => {
            ...
            df.querySelector(".close-box").addEventListener(
                "click",
                function () {
                    df.classList.remove("active");
                    // -----------------
        ...
        pSona.querySelectorAll(".bx-sn-bt").forEach((c_btn) => {
            c_btn.addEventListener("click", function (c) {
                ...
                let content = this.dataset.boxcontent,
                    childrenSona = this.parentElement.parentElement
                        .parentElement,
                    element_sona_content = childrenSona.querySelector(
                        `${content}`
                    ),
                ...
                // -----------------
                element_sona_content
                    .querySelectorAll(".close-box")
                    .forEach((cl_bx) => {
...
function historyDate() {
    try {
        ...
        document
            .querySelector(".hejry-time .calender-hejry")
            .insertAdjacentHTML(
                "beforeend",
                `
...
function fahrsAndBest() {
    const fahrs_btn = document.querySelector(
            ".bottom-nav .link-nav.fahrs-links"
        ),
